...
# GAMEFLEET_DEV=true is for working on GameFleet: API docs are served, logging is verbose and a dashboard
# without configured users is open to everyone. Anything else is production, where none of that holds.
DEV = os.getenv("GAMEFLEET_DEV", "").strip().lower() in ("1", "true", "yes", "on")
